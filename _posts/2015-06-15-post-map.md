...
There are a bunch of solutions for adding more geographical information to a post.  The common WordPress-style solution is to keep location data alongside the post using tags or categories and to generate these manually, as part of the posts' creation.  This involves some kind of manual entry -- either by adding categories or via shortcodes.  I'd much rather work on an automated solution than a manual one.
...
Another problem is performance: rendering 70+ markers is not fast.  It's not impossibly slow, but it's enough of a problem to seek other solutions

![Solution showing all the markers](../public/img/map-all-posts.png "Solution showing all the markers")

The page is [here][render-posts].
...
The page is [here][render-post-clusters].

#### DBScan Clustering with Route Inference

Since I wanted to cluster posts which were not only closely in space but also in time, I needed to control the clustering function.  My first stop was [*k-means*][k-means], which I've loved since grad school for its intuitive methodology.  For this purpose, though, it has a flaw: you have to guess the number of clusters. While I could have come up with an estimate (say by counting the number of unique cities visited), I liked the idea of the algorithm doing that work for me.
